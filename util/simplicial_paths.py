...
class SimplicialComplex:
    def __init__(self, pts, weighted_edges=False, max_extra=10):
        self.nodes = pts
    ...
    def plot(self, ax=None):
        ...
        plt.scatter(self.nodes[active_nodes,0], self.nodes[active_nodes,1], color='black', s=1.5)
    ...
    def compute_proj_matrix(self, thresh=1e-12):
        ...
        H = eig_v[:,:num_holes].T
        H_extra = eig_v[:,:num_holes+self.max_extra].T

        self.H, self.H_extra, self.num_holes = H, H_extra, num_holes
    ...
    def extra_proj(self, x, num_extra):
        x_hat = x[self.edge_vec.astype(bool)] # consider only edges in edge set
        y = (self.H_extra[:self.num_holes + num_extra,:] @ x_hat).T
        return y
