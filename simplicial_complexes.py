...
class SimplicialComplex:
    def __init__(self, pts):
        ...
        # initialize node, edge, and face vectors
        self.simplex_vectors_init(self, num_nodes, num_edges, num_faces)

        # initialize incidence matrices (assuming all simplices active)
        self.incidence_matrices_init()

    # initialize node_vec, edge_vec, and face_vec
    def simplex_vectors_init(self, num_nodes, num_edges, num_faces):
        num_nodes, num_edges, num_faces = len(self.nodes), len(self.edges), len(self.faces)
    ...
    # initialize incidence matrices (assuming all simplices active)
    def incidence_matrices_init(self):
        ...
        self.B0_main, self.B1_main = B0, B1
    ...
    def plot(self):
        for i, face in enumerate(self.faces):
            ...
            ti = plt.Polygon(tri, color='white')
        ...
        for i, edge in enumerate(self.edges):
            ...
            plt.plot(line[:,0], line[:,1], color='black', linewidth=1, alpha=0.5)

        active_nodes = self.node_vec.astype(bool)

        plt.scatter(self.nodes[active_nodes,0], self.nodes[active_nodes,1], color='blue', alpha=0.5)
    ...
    def plot_trajectories(self, trajectories):
        ...
        for i, trajectory in enumerate(trajectories):
            ...
            for edge in trajectory:
                ...
                plt.plot(line[:,0], line[:,1], color=c, linewidth=3)

            start_end_nodes = np.array([trajectory[0][0], trajectory[-1][-1]])

            plt.scatter(self.nodes[start_end_nodes, 0], self.nodes[start_end_nodes, 1], color='k', marker='x', linewidths=5)
    ...
    def harm_proj(self, x, thresh=1e-12):
        ...
        x_hat = x[self.edge_vec.astype(bool)]

        y = (H.T @ np.diag(x_hat)).T
