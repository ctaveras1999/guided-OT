...
class Trajectory:
    def __init__(self, SC, coords, num_trajectories, num_nodes):
        ...
        for i in range(num_trajectories):
            ...
            for j in range(len(skeleton)-1):
                path = nx.shortest_path(self.G, skeleton[j], skeleton[j+1])
    ...
    def project_trajectories(self):
        ...
        for _, trajectory in enumerate(self.all_trajectories):
            ...
            for i, edge in enumerate(trajectory):
                x_vec = self.trajectory_to_edgevec([edge])
                edge_proj = self.SC.harm_proj(x_vec)
                trajectory_projs.append(edge_proj)

            x_vec = self.trajectory_to_edgevec(trajectory)
            harm_proj_path = self.SC.harm_proj(x_vec)
            print(harm_proj_path)
    ...
    def plot_trajectory_proj(self, class_name, color):
        ...
        for _, trajectory_proj in enumerate(proj_res_path):
            projections = np.cumsum(trajectory_proj, axis=0)
        ...
        plt.scatter(all_projections[:,0], all_projections[:,1], c=color, marker=',', s=7)
        plt.scatter(proj_res_val[:,0], proj_res_val[:,1], c=color, label=class_name, marker='*', s=200)
